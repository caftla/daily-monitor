...
const predict = require('../anomaly-detection/index');

export default async function (_data, {dateTo, timeZoneOffset}) {
    const trace = (x, y) => {
        console.log(x);
        return y
    };
    ...
    // predict      :: [{time, value}] -> Promise [{ds, yhat, ...}]
    // makePredict  :: [{row, sale, ...}] -> Pvromise {row: {yhat? ...}}
    const makePredictions = xs => Promise.resolve(
        R.pipe(
            R.map(y => [y.ds, y]),
            R.fromPairs
        )([{ds: xs[0].row, yhat: Math.random()}])) // x => trace('--->' + JSON.stringify(x) + '<----', x)

    const three = (f, p, g) => x => p(f(x)).then(g);
    const groupAll = async (hash) => {
        const tuples = await Promise.all(
            R.pipe(
                R.toPairs,
                R.map(([k, p]) => p.then(r => [k, r]))
            )(hash)
        );
        return R.fromPairs(tuples)
    };

    const pagesPromises = R.pipe(
        R.groupBy(x => x.page) // country
        , R.map(R.groupBy(x => x.row)) // day
        , R.map(
            three(R.pipe(
                R.map(sumAll) // calculate sum of metrics per country per day (by ignoring affiliates)
                , R.toPairs
                , R.map(([row, metrics]) => R.merge({row}, ratios(metrics))))

                , xs => makePredictions(xs).then(anomalies => xs.map(x => R.merge(x, anomalies[x.row])))
                // , X => X
                , R.pipe(R.applySpec({
    ...
    const pages = await groupAll(pagesPromises);
